...
type IAuthor interface {
	...
	FindAllWithBooks(ctx context.Context, db *sql.DB, bookUseCase IBook) ([]vos.AuthorWithBooks, error)
	FindById(ctx context.Context, db *sql.DB, id string) (*entity.Author, error)
	FindByIdWithBooks(ctx context.Context, db *sql.DB, id string, bookUseCase IBook) (*vos.AuthorWithBooks, error)
